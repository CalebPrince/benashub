...
| Forgot password | `/account/forgot-password` | Emails a single-use reset link (expires in 1 hour) |
...
- **Emails (SMTP)**: configure your mail server under Admin > Settings > Email (SMTP) — works with Gmail, Zoho, or your hosting provider's mail server, and there's a "Send Test" button to verify. Once set, the site sends a welcome email on customer registration, an order confirmation to the customer when payment succeeds, a shipped notification when an order's status is changed to "shipped" in the admin panel, password reset links, and (optionally) a new-order notification to the admin email. Leave the SMTP host empty to disable all emails (password reset will tell customers to contact you instead).
